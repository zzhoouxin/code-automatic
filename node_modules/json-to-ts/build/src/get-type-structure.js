...
function createTypeDescription(typeObj, isUnion) {
    if (util_1.isArray(typeObj)) {
        return {
            id: Hash(JSON.stringify(typeObj.concat([isUnion]))),
...
function hasSamePrimitiveElements(a, b) {
...
function generateTypeName(str, index) {
    if (index === void 0) { index = 0; }
    var postFix = index === 0 ? '' : index;
    return str.charAt(0).toUpperCase() + str.slice(1) + postFix;
}
function createTypeObject(obj, types) {
    return Object.entries(obj)
        .reduce(function (typeObj, _a) {
        var _b;
        var key = _a[0], value = _a[1];
        var rootTypeId = getTypeStructure(value, types).rootTypeId;
        return __assign({}, typeObj, (_b = {}, _b[key] = rootTypeId, _b));
    }, {});
}
function getMergedObjects(typesOfArray, types) {
    var typeObjects = typesOfArray
        .map(function (typeDesc) { return typeDesc.typeObj; });
    ...
    var typeObj = allKeys
        ...
        return __assign({}, obj, (_a = {}, _a[keyValue] = type, _a));
    }, {});
...
function getMergedUnion(typesOfArray, types) {
    var innerUnionsTypes = typesOfArray
        ...
        .reduce(function (a, b) { return a.concat(b); }, []);
    var primitiveTypes = typesOfArray
        .filter(function (id) { return !util_1.findTypeById(id, types) || !util_1.findTypeById(id, types).isUnion; }); // primitives or not union
    return getIdByType(innerUnionsTypes.concat(primitiveTypes), types, true);
}
function getInnerArrayType(typesOfArray, types) {
    var containsNull = typesOfArray.includes('null');
    var arrayTypesDescriptions = typesOfArray
        .map(function (id) { return util_1.findTypeById(id, types); })
        .filter(function (_) { return !!_; });
    var allArrayType = arrayTypesDescriptions
        .filter(function (typeDesc) { return util_1.getTypeDescriptionGroup(typeDesc) === model_1.TypeGroup.Array; })
        .length === typesOfArray.length;
    var allArrayTypeWithNull = arrayTypesDescriptions
        .filter(function (typeDesc) { return util_1.getTypeDescriptionGroup(typeDesc) === model_1.TypeGroup.Array; })
        .length + 1 === typesOfArray.length && containsNull;
    var allObjectTypeWithNull = arrayTypesDescriptions
        .filter(function (typeDesc) { return util_1.getTypeDescriptionGroup(typeDesc) === model_1.TypeGroup.Object; })
        .length + 1 === typesOfArray.length && containsNull;
    var allPrimitiveType = arrayTypesDescriptions.length === 0;
    var allObjectType = arrayTypesDescriptions
        .filter(function (typeDesc) { return util_1.getTypeDescriptionGroup(typeDesc) === model_1.TypeGroup.Object; })
        .length === typesOfArray.length;
    var canBeMerged = arrayTypesDescriptions.length > 0 && (allArrayType || allObjectType);
...
types) {
    ...
    switch (getTypeGroup(targetObj)) {
        case model_1.TypeGroup.Array:
            var typesOfArray = targetObj
                .map(function (_) { return getTypeStructure(_, types).rootTypeId; })
                .filter(util_1.onlyUnique);
        ...
        case model_1.TypeGroup.Primitive:
            var simpleType = getSimpleTypeName(targetObj);
            return {
        ...
        switch (util_1.getTypeDescriptionGroup(typeDesc)) {
            case model_1.TypeGroup.Array:
                var arrSubTypes = typeDesc.arrayOfTypes
                    ...
                    .reduce(function (a, b) { return a.concat(b); }, []);
                return [typeDesc.id].concat(arrSubTypes);
            case model_1.TypeGroup.Object:
                var objSubTypes = Object.values(typeDesc.typeObj)
                    ...
                    .reduce(function (a, b) { return a.concat(b); }, []);
                return [typeDesc.id].concat(objSubTypes);
        }
    };
    return subTypes(typeDesc);
}
function optimizeTypeStructure(typeStructure) {
    ...
    var optimizedTypes = typeStructure.types
        .filter(function (typeDesc) { return usedTypeIds.includes(typeDesc.id); });
